	...
	ui(new Ui::MainWindow)
{
    ...
    connect(PortDialog,SIGNAL(Motor_connect_status(int)),this,SLOT(Motor_status(int)));
    ...
    connect(PortDialog,SIGNAL(Motor_Port_close()),this,SLOT(program_close()));
    ...
    timer_portClose = new QTimer(this);
    connect(timer_portClose,SIGNAL(timeout()),this,SLOT(P_M_close()));
...
void MainWindow::Create_statusbar()
{
    ...
    motor_state->setMinimumSize(150,22);
	...
	storenum = new QLabel;
    storenum->setMinimumSize(910,22);
...
void MainWindow::connectADQDevice()
{
	...
	int n_of_ADQ214 = ADQControlUnit_NofADQ214(adq_cu);				//返回找到ADQ214设备的数量
...
void MainWindow::Motor_status(int a)
{
    ...
    case 1:
        ...
        break;
    case 2:
        motor_state->setText(QString::fromLocal8Bit("串口已连接，电机未打开"));
        break;
    case 3:
        motor_state->setText(QString::fromLocal8Bit("串口已连接，电机已打开"));
        break;
    default:
        break;
    }
...
void MainWindow::program_close()
{
    qDebug() << "PortMotorClose = true";
    PortMotorClose = true;
}
...
bool MainWindow::adq_para_set()
{
	...
	if(ADQ214_SetTriggerMode(adq_cu,1,trig_mode) == 0)
		return false;
	if(trig_mode == 3)							//电平触发
	{
		qint16 trig_level = mysetting.trigLevel;
		if(ADQ214_SetLvlTrigLevel(adq_cu,1,trig_level) == 0)
			return false;
		int trig_flank = 1;						//触发边沿 -> 上升沿
		if(ADQ214_SetLvlTrigFlank(adq_cu,1,trig_flank) == 0)
			return false;
		int trig_channel = 1;					//触发通道:1通道A,2通道B
		if(ADQ214_SetLvlTrigChannel(adq_cu,1,trig_channel) == 0)
			return false;
	}

	if(mysetting.isPreTrig)
	{
		if(ADQ214_SetPreTrigSamples(adq_cu,1,mysetting.Pre_OR_HoldOff_Samples) == 0)
			return false;
	}
	else
	{
		if(ADQ214_SetTriggerHoldOffSamples(adq_cu,1,mysetting.Pre_OR_HoldOff_Samples) == 0)
			return false;
	}

	int clock_source = 0;						//时钟源选择0，内部时钟，内部参考
	if(ADQ214_SetClockSource(adq_cu,1,clock_source) == 0)
		return false;
	int pll_divider = int(800/mysetting.sampleFreq);
	if(ADQ214_SetPllFreqDivider(adq_cu,1,pll_divider) == 0)
		return false;
	number_of_records = mysetting.plsAccNum;	//脉冲数
	samples_per_record = mysetting.sampleNum;	//采样点数
	if(ADQ214_MultiRecordSetup(adq_cu,1,number_of_records,samples_per_record) == 0)
		return false;
	return true;
}
...
{
	if(ADQ214_DisarmTrigger(adq_cu,1) == 0)			//Disarm unit
		return false;
	if(ADQ214_ArmTrigger(adq_cu,1) == 0)			//Arm unit
		return false;
	...
	do
	{
		...
		trigged = ADQ214_GetTriggedAll(adq_cu,1);					//Trigger unit
...
void MainWindow::single_upload_store()
{
	...
	for(uint i = 0;i < number_of_records;i++)
	{
		QCoreApplication::processEvents();
		int rn1 = 0;
		ADQ214_GetData(adq_cu,1,target_buffers,samples_per_record,
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    ...
    else
    {
        ...
        PortDialog->close_order();
        qDebug() << "PortMotorClose ===" << PortMotorClose;
        timer_portClose->start(PERIOD_OF_JUDGE);
    }

    event->ignore();
}

void MainWindow::P_M_close()
{
    qDebug() << "P_M_Close";
    if(PortMotorClose)
    {
        timer_portClose->stop();            //关闭定时器
        DeleteADQControlUnit(adq_cu);       //断开采集卡连接
        exit(0);                            //关闭程序
    }
}
